...
with open('points_1a.npy', 'rb') as f: #kayıtlı dosyadan noktalarımzı açıyoruz.
    points_1 = np.load(f,allow_pickle=True)

with open('points_2a.npy', 'rb') as f:  #kayıtlı dosyadan noktalarımzı açıyoruz.
...
for i in range(0,len(test_points)):
    result = np.dot(w,test_points[i][0]) # ağırlıklarımız ile x leri çarpıyoruz.
